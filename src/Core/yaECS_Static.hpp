...
namespace ECS_Static
{
    ...
    */
    ...
    public:
        ...
        */
        ...
        {
            return std::tuple<std::vector<Ts>&...>(std::get<std::vector<Ts>>(m_components)...);
        }
    ...
    /*
        An array of views to queries with specified archetypes
        Does not contain full info about types
    */
    template<size_t SIZE, typename... Comps>
    struct ViewQuery
    {
        std::array<std::tuple<std::vector<Comps>&...>, SIZE> m_view;
    };
    ...
    */
    ...
    struct Query
    {
        ...
        template<typename... Ts>
        constexpr inline auto makeView()
        {
            return std::array<std::tuple<std::vector<Ts>&...>, std::tuple_size_v<decltype(m_tpl)>>{ std::get<Args>(m_tpl).getView<Ts...>() ... };
        }
    };
